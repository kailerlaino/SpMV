...
Since GPUs are best for fixed-size operations, with some architectures not even supporting arbitrary loops~\cite{redhat}, sparse matrix operations can be difficult to compute efficiently on GPUs, as they might require a variable number of computations based on which part of the matrix is being operated on.

\section{Expected Results}

The end results this project aims for is correct and performant implementations of matrix-vector multiplication CUDA kernels that utilize both state of the art and previously state of the art sparse matrix data structures and algorithms for multiplication. This will finally be comprise of:
